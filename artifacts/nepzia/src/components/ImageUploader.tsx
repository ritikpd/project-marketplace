...
function validateFile(file: File, errType: string, errSize: string): string | null {
  if (!ACCEPTED_TYPES.includes(file.type)) return errType;
  if (file.size > MAX_SIZE_MB * 1024 * 1024) return errSize;
  return null;
}
...
export function ImageUploader({ images, onChange, disabled }: ImageUploaderProps) {
  ...
  const processFiles = useCallback(async (files: File[]) => {
    const errType = t("uploader.errType");
    const errSize = t("uploader.errSize");
    const errFailed = t("uploader.errFailed");
    const remaining = MAX_IMAGES - images.filter(i => !i.error).length;
    const toProcess = files.slice(0, remaining);

    const validFiles: { file: File; preview: string }[] = [];
    for (const file of toProcess) {
      const err = validateFile(file, errType, errSize);
      if (err) {
    ...
    for (let i = 0; i < validFiles.length; i++) {
      ...
      try {
        ...
        const msg = err instanceof Error ? err.message : errFailed;
        onChange(prev => {
          const next = [...prev];
          const idx = next.findIndex(x => x.previewUrl === preview);
      ...
      {!hasImages ? (
        ...
        >
          ...
          <div className="text-center">
            <p className="text-white font-semibold">{t("uploader.uploadPhotos")}</p>
            <p className="text-muted-foreground text-sm mt-1">{t("uploader.dragDrop")}</p>
            <p className="text-muted-foreground text-xs mt-1">{t("uploader.hint")}</p>
          </div>
          <Button
            type="button"
            disabled={disabled}
        ...
        <>
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            ...
            {canAddMore && (
              ...
              >
                ...
                <span className="text-muted-foreground text-xs">{t("uploader.addMore")}</span>
          ...
          <p className="text-muted-foreground text-xs">
            {t("uploader.dragToReorder", { current: images.filter(i => !i.error).length, max: MAX_IMAGES })}
          </p>
        </>
      )}
